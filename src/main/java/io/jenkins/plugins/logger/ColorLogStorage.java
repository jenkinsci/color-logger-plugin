 ...
 * @author albert.lv
 */
public class ColorLogStorage implements LogStorage {

  private static final Logger LOGGER = Logger.getLogger(ColorLogStorage.class.getName());

  private static final Map<File, ColorLogStorage> openStorages = Collections
      .synchronizedMap(new HashMap<>());

  private static final int LF = 0x0A;

  public static synchronized LogStorage forFile(File log) {
    return openStorages.computeIfAbsent(log, ColorLogStorage::new);
  }
  ...
  public ColorLogStorage(File log) {
    this.log = log;
  ...
  @Override public BuildListener overallListener() throws IOException, InterruptedException {
    return new BufferedBuildListener(new ColorLogStorage.IndexOutputStream(null));
  }

  @Override public TaskListener nodeListener(FlowNode node) throws IOException, InterruptedException {
    return new BufferedBuildListener(new ColorLogStorage.IndexOutputStream(node.getId()));
  }
  ...
  private final class IndexOutputStream extends OutputStream {
    ...
    @Override public void write(int b) throws IOException {
      synchronized (ColorLogStorage.this) {
        checkId(id);
        writeColorLog(b);
      }
    }
    ...
    private void eol(byte[] bytes, int len) throws IOException {
      if (LOGGER.isLoggable(Level.FINE)) {
        LOGGER.log(Level.FINE, "ColorPipelineLogFilter: I am working");
      }
  ...
  @Override public AnnotatedLargeText<Executable> overallLog(FlowExecutionOwner.Executable build, boolean complete) {
    maybeFlush();
    return new AnnotatedLargeText<FlowExecutionOwner.Executable>(log, StandardCharsets.UTF_8, complete, build) {
      @Override public long writeHtmlTo(long start, Writer w) throws IOException {
        try (BufferedReader indexBR = index.isFile() ? Files.newBufferedReader(index.toPath(), StandardCharsets.UTF_8) : new BufferedReader(new NullReader(0))) {
          ConsoleAnnotationOutputStream<Executable> caos = new ConsoleAnnotationOutputStream<>(w, ConsoleAnnotators
